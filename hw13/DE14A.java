// DE14A.java CS5125/6025 Cheng 2020
...
// Usage: java DE14A fib512.txt < BMP image > encoded
...
public class DE14A {
  ...
  void yuvTransform() { // RGB to YUV
    for (int i = 0; i < height; i++)
      for (int j = 0; j < width; j++) {
        yuv[i][j][0] = (int) ((65.738 * raw[i][j][2] + 129.057 * raw[i][j][1] + 25.064 * raw[i][j][0]) / 256 + 16);
        yuv[i][j][1] = (int) ((-37.945 * raw[i][j][2] - 74.494 * raw[i][j][1] + 112.439 * raw[i][j][0]) / 256 + 128);
        yuv[i][j][2] = (int) ((112.439 * raw[i][j][2] - 94.154 * raw[i][j][1] - 18.285 * raw[i][j][0]) / 256 + 128);
      }
  }
  ...
  void uvSubsampling() {
    for (int i = 0; i < height / 2; i++)
      for (int j = 0; j < width / 2; j++)
        for (int k = 1; k < 3; k++)
          yuv[i][j][k] = (yuv[2 * i][2 * j][k] + yuv[2 * i][2 * j + 1][k] + yuv[2 * i + 1][2 * j][k]
              + yuv[2 * i + 1][2 * j + 1][k]) / 4;
  }
  ...
  void rowTransform(int height, int width, int k) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < halfWidth - 1; j++)
        raw[i][2 * j + 1][k] = yuv[i][2 * j + 1][k] - (yuv[i][2 * j][k] + yuv[i][2 * j + 2][k]) / 2;
      raw[i][width - 1][k] = yuv[i][width - 1][k] - yuv[i][width - 2][k];
      for (int j = 1; j < halfWidth; j++)
        raw[i][2 * j][k] = yuv[i][2 * j][k] + (raw[i][2 * j - 1][k] + raw[i][2 * j + 1][k] + 2) / 4;
  ...
  void columnTransform(int height, int width, int k) {
    ...
    for (int j = 0; j < width; j++) {
      for (int i = 0; i < halfHeight - 1; i++)
        raw[2 * i + 1][j][k] = yuv[2 * i + 1][j][k] - (yuv[2 * i][j][k] + yuv[2 * i + 2][j][k]) / 2;

      raw[height - 1][j][k] = yuv[height - 1][j][k] - yuv[height - 2][j][k];
      
      for (int i = 1; i < halfHeight; i++)
        raw[2 * i][j][k] = yuv[2 * i][j][k] + (raw[2 * i - 1][j][k] + raw[2 * i + 1][j][k] + 2) / 4;
      ...
      for (int i = 0; i < borders[k][0]; i++)
        for (int j = 0; j < borders[k][1]; j++) {
          if (i > borders[k][2] || j > borders[k][3]) {
            if (yuv[i][j][k] > 0 && yuv[i][j][k] <= threshold)
              yuv[i][j][k] = 0;
            if (yuv[i][j][k] < 0 && yuv[i][j][k] >= -threshold)
              yuv[i][j][k] = 0;
            if (yuv[i][j][k] != 0)
  ...
  public static void main(String[] args) {
    if (args.length < 1) {
      System.err.println("Usage: java DE14A fib512 < in.bmp > out");
      return;
    }
    DE14A de14 = new DE14A();
    de14.readCode(args[0]);
    de14.readHeader();
    de14.readImage();
    de14.yuvTransform();
    de14.uvSubsampling();
    de14.waveletTransform();
    de14.shrink();
    de14.encode();
  }
}
